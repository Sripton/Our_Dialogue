...
✅ Решение: Передавать user_id через query-параметр или через сессию
🔹 Если ты используешь сессии (и всё работает):

const userID = req.session.userID;
ОК, можно оставить как есть, если req.session.userID существует (важно это проверить с логом!).

🔹 Если сессии могут отсутствовать (например, API вызывается без авторизации), лучше:

const { user_id } = req.query;
И тогда вызов с клиента будет:
...
Кеширование (cache) — это техника временного хранения часто используемых данных, 
чтобы не запрашивать их снова и снова из БД или API, а получить быстрее из памяти или локального хранилища.

🔧 В контексте  Navbar:
Для отображения счётчика «📩 Ответили» в меню.
Если на каждый рендер Navbar делать fetch('/api/useractivity/:id') — это:

🔁 Повторяет одни и те же запросы;
⏳ Замедляет рендер;
📡 Нагрузит сервер.

✅ Как кешировать в React
🔸 Вариант 1: Кеш в памяти (useRef + useState)
...
useEffect(() => {
  ...
  fetch(`/api/useractivity/${userIDsession}`)
    ...
    .then((data) => {
      ...
      cache.current = data; // кешируем результат
    });
}, [userIDsession]);
